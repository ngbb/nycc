...
split_by_sentence <- function (text, removereal = TRUE) {
  ...
  # remove "real" captions
  if(removereal) {
    result <- result[!str_detect(all_text, result)]
  }
...
# tweet image + caption
sendTweet <- function() {
  gencap1 <- makeCaption(split_by_sentence(babble(captionNgram, genlen = 1500)))
